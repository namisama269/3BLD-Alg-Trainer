...
var GAN_CIC_LIST = Array(256).fill(void 0).map((v, i) => i << 8 | 1);
...
var GanGen2ProtocolDriver = class {
  ...
  async handleStateEvent(conn, eventMessage) {
    ...
    if (eventType == 1) {
      ...
      let diff = this.lastSerial == -1 ? 1 : Math.min(serial - this.lastSerial & 255, 7);
...
var GanGen3ProtocolDriver = class {
   ...
   */
  isSerialInRange(start, end, serial) {
    return (end - start & 255) > (serial - start & 255) && (start - serial & 255) > 0 && (end - serial & 255) > 0;
  }
  ...
  injectMissedMoveToBuffer(move) {
    if (move.type == "MOVE" && this.moveBuffer.length > 0) {
      ...
      if (move.serial == (bufferHead.serial - 1 & 255)) {
        move.cubeTimestamp = bufferHead.cubeTimestamp - 10;
        ...
        for (let i2 = 0; i2 < count; i2++) {
          ...
          if (face >= 0) {
            ...
            this.injectMissedMoveToBuffer({
              ...
              // Missed and recovered events has no meaningfull local timestamps
              cubeTimestamp: 0,
              // Actual cube hardware timestamp will be adjusted on insertion to FIFO buffer
              face,
              direction,
              move: move.trim()
            });
...
async function connectGanCube(customMacAddressProvider) {
  ...
  var device = await navigator.bluetooth.requestDevice({
    ...
    optionalServices: [GAN_GEN2_SERVICE, GAN_GEN3_SERVICE],
